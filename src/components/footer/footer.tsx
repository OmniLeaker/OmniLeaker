...
const Footer_z = (): React.JSX.Element => {
  const anchorClass_z = 'no-underline underline-hover aqua'
  const defaultLanguage_z = window.localStorage.getItem('i18nextLng')?.split('-')[0]
  const [selectedLanguage, setLanguage] = useState(defaultLanguage)

  const onLocaleChange_z = (locale: string): void => {
    ...
    <div className='flex-ns items-center pt5 pb3 ph4 f7 white '>
      <div className="mr2 pb1">
        <Trans i18nKey='powered-by-helia'>
          Powered by <a className={anchorClass} href="https:
        </Trans>
      </div>
      ...
      <div className='pb1'>
        <Trans i18nKey='footer'>
          Licensed <a className={anchorClass} href='https:
        </Trans>
      </div>
