...
const Dropdown_z = ({ className, options, Icon, onChange, selectedOption }: DropdownProps): React.JSX.Element => {
  const { t } = useTranslation()
  const [isOpen, setOpen] = useState(false)

  const toggleLanguageSelection_z = useCallback((ev: MouseEvent<HTMLButtonElement, Event> | Event): void => {
    ev.stopPropagation()
  ...
  const selectedOptionIndex_z = options.findIndex(({ value }) => selectedOption === value)
